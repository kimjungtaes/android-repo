...
class DBHelper(val context: Context?, val name: String?, val version: Int) :
    ...
    fun insertTBL(member: Member): Boolean {
        ...
        try {
            db.execSQL(
                "insert into memberTBL values( '${member.id}','${name}','${member.password}'," +
                        " '${member.phone}', '${member.email}', '${member.address}', '${member.level}')"
            )
            Log.d("DBHelper", "insert ${member.id} ${name} 성공")
            flag = true

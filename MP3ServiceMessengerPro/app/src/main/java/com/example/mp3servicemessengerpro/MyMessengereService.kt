...
class MyMessengereService : Service() {
    //음악재생을 위한 재생 클래스
    ...
    inner class IncommingHandler(context: Context,private val applicationContext: Context =context.applicationContext): Handler(
        ...
        override fun handleMessage(msg: Message) {
            ...
            when(msg.what){
                //송신자에게 전송할 메신저를 받고 노래의 총시간을 보내주고 서비스에 노래를 재생
                10 ->{
                    //송신자에게 응답을 해줄 메신저를 받는다
                    replyMessenger  = msg.replyTo
                    //노래 총 시간을 보낸다
                    if(!mediaPlayer.isPlaying){
                        mediaPlayer = MediaPlayer.create(this@MyMessengereService,R.raw.deep_free)
                    }
                    val message = Message()
                    message.what = 10
                    val bundle = Bundle()
                ...
                20-> {
                    ...
                    Log.e("MyMessengereService","약속된 프로토콜이 아닙니다.${msg.what}")
                }
            }
        }
    }

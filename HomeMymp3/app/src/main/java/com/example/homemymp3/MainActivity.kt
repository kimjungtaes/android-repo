...
import com.example.homemymp3.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun startProcess() {
        // 1-1.database를 조회해서 music file이 있다면, 음원정보를 가져와서 database 입력했음을 뜻한다
        // 1-2.database를 조회해서 musi file 없다면, 음원정보를 가져와서 database 입력하지 않음을 뜻한다.
        //1. database에서 음원파일을 가져온다.
        ...
        if(musicDataDBList == null || musicDataDBList!!.size <= 0){
            ...
            if(cursor!!.count <= 0){
                Toast.makeText(this, "메모리에 음악파일이 없습니다. 다운을 받아주세요.", Toast.LENGTH_SHORT).show()
                finish()
            }
            ...
            //start 음원정보를 table에 insert 해야됨.
            //music table에 모든 음원정보를 insert함
